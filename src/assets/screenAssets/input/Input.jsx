...
const Input = ({ err, setErr, email, type, name, inputClass, formik, fp, setInputVal }) => {
    ...
    return (
        <div className='signUpRowDiv'>
            <div className={ !type ? '' : passBorder ? 'signUpPassDiv signUpBorder' : 'signUpPassDiv'}>
                <input type={type ? passType : 'text'} onBlur={() => hideLabel(false)} onFocus={() => hideLabel(true)} name={name?.toLowerCase()} value={ formik ? formik?.values[name?.toLowerCase()] : email} onChange={(e) => {
                    if(formik)
            ...
            {<p className='signUpError'>{ formik.values[name.toLowerCase()].trim() != '' ? 
